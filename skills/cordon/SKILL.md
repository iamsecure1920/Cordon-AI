...
name: cordon
description: >
  Orchestrate an authorized VAPT engagement end to end with the Cordon MCP
  server: authorize, recon, expand, scan, takeover, triage, validate, report.
  Use when the user asks to run Cordon, hunt a bug bounty target, assess an
...
1. `cordon_load_scope(scope_path=...)`. Without it every target-taking tool
...
- Long scans return a `job_id`. Poll `job_status`, then read results with
  ...
  a large result set.
- Check `cordon_status()` for remaining budget before starting an expensive
