...
No scope loaded. Call `cordon_load_scope(scope_path="scope.yaml")`.

## A tool returns `tool_unavailable`

The binary is not installed. `cordon_capabilities()` shows what is registered;
`cordon doctor` on the command line shows what is actually present. Grouped
