...
``config.yaml`` controls *how* EasyHunt runs. ``scope.yaml`` controls *what it may
...
Resolution order for each file: explicit argument, then ``$EASYHUNT_CONFIG`` /
``$EASYHUNT_SCOPE``, then the current directory, then ``~/.config/easyhunt/``.
"""
...
_SEARCH_DIRS = [Path.cwd(), Path.home() / ".config" / "easyhunt"]
...
def find_scope(explicit: str | Path | None = None) -> Path | None:
    if explicit:
        ...
        return path
    return _search(["scope.yaml"], "EASYHUNT_SCOPE")
