...
EasyHunt never registers a resource on a third-party provider. ``takeover_poc_plan``
...
from easyhunt.tools.common import HOST_PATTERN, register_spec, run_one, split_targets
...
async def takeover_verify(target: str) -> dict[str, Any]:
    ...
    return {
        ...
        "next_step": (
            ...
            "that claiming the resource is a human action, not an EasyHunt one."
            if verified
...
# --------------------------------------------------------------------------- #
# PoC — planned by EasyHunt, executed by a human, recorded here
# --------------------------------------------------------------------------- #
...
async def takeover_poc_plan(target: str) -> dict[str, Any]:
    ...
    EasyHunt does not register resources at third-party providers on your behalf.
    ...
    proof_path = f"/.well-known/easyhunt-poc-{handle}-{nonce}.txt"
    ...
    return {
        ...
        "proof_content": (
            f"EasyHunt subdomain takeover proof of concept\n"
            f"host: {host}\n"
            f"researcher: {handle}\n"
            f"nonce: {nonce}\n"
...
async def takeover_confirm(target: str, proof_url: str, notes: str = "") -> dict[str, Any]:
    ...
    proof_is_live = response.status_code == 200 and "EasyHunt subdomain takeover proof" in body
    ...
    finding.confirm(
        PoC(
            ...
            expected_result=f"HTTP 200 with the EasyHunt proof content at {proof_url}",
