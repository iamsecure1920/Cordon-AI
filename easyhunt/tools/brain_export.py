"""``easyhunt brain export`` — the neuron brain as a self-contained HTML pop-up.
...
(``~/.easyhunt/brain-activity.jsonl``) is read and embedded into the page, so
...
Usage:
    ...
    easyhunt brain export                 # writes brain.html + brain-replay.json
    easyhunt brain export --out /tmp/x    # write to /tmp/x.html and /tmp/x.json
    easyhunt brain export --open          # open the page in the browser
"""
...
<head>
<meta charset="utf-8">
<title>EasyHunt Neuron Brain — live neural activity</title>
<style>
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="easyhunt brain export",
        description="Self-contained HTML neural animation of the neuron brain's sensed activity.",
    )
    parser.add_argument("--path", help="activity feed path (default ~/.easyhunt/brain-activity.jsonl)")
