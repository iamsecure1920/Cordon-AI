"""``easyhunt brain watch`` — the neuron brain's live neural animation.
...
appended as one JSON line to the activity feed (``~/.easyhunt/brain-activity.jsonl``
...
``easyhunt serve``:
    ...
    easyhunt brain watch            # follow the default activity feed
    easyhunt brain watch --path x   # follow a specific feed
    easyhunt brain watch --once     # one frame, then exit (for CI/demos)
