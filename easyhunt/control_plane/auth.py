...
An EasyHunt server reachable over a network without authentication is remote code
...
**It authenticates.** EasyHunt acts as an OAuth 2.1 *Resource Server* (RFC 9728):
...
**It authorizes, against EasyHunt's own risk model.** This is the part worth the
...
``easyhunt:approve`` responding to approval prompts
``easyhunt:admin``   loading a scope, re-pinning tool definitions
===================  ====================================================
...
``easyhunt:approve`` is deliberately separate from everything else. If the agent's
...
* ``jwt`` — you already run an IdP (Auth0, Keycloak, Entra, Okta). EasyHunt
...
log = logging.getLogger("easyhunt.auth")
...
MODE_SCOPES: dict[str, str] = {
    "passive": "easyhunt:recon",
    "aggressive": "easyhunt:scan",
    "exploit": "easyhunt:exploit",
}
...
TOOL_SCOPE_OVERRIDES: dict[str, str] = {
    "approval_respond": "easyhunt:approve",
    "easyhunt_load_scope": "easyhunt:admin",
    "rules_reload": "easyhunt:admin",
    "easyhunt_finish": "easyhunt:admin",
    # Read-only introspection: useful to grant broadly.
    "easyhunt_status": "easyhunt:read",
    "easyhunt_capabilities": "easyhunt:read",
    "scope_check": "easyhunt:read",
    "audit_tail": "easyhunt:read",
    "findings_list": "easyhunt:read",
    "finding_detail": "easyhunt:read",
    "rules_list": "easyhunt:read",
    "job_status": "easyhunt:read",
    "job_list": "easyhunt:read",
    "taskgraph_next": "easyhunt:read",
    "approval_pending": "easyhunt:read",
}
...
def scopes_for_tool(name: str, mode: str | None) -> list[str]:
    ...
    control-plane tool nobody classified — ``easyhunt:admin``. That last fallback
...
class AuthConfig:
    ...
    upstream_client_id_env: str = "EASYHUNT_OAUTH_CLIENT_ID"
    ...
    upstream_client_secret_env: str = "EASYHUNT_OAUTH_CLIENT_SECRET"  # noqa: S105
    ...
    def from_dict(cls, data: dict[str, Any] | None) -> AuthConfig:
        ...
        return cls(
            ...
            upstream_client_id_env=str(data.get("upstream_client_id_env") or "EASYHUNT_OAUTH_CLIENT_ID"),
            upstream_client_secret_env=str(
                data.get("upstream_client_secret_env") or "EASYHUNT_OAUTH_CLIENT_SECRET"
            ),
...
def assert_bind_is_safe(host: str, *, transport: str, auth: AuthConfig) -> None:
    ...
    if not auth.enabled:
        raise ConfigError(
            ...
            "A network-reachable EasyHunt server executes security tools on "
            ...
            "See `easyhunt doctor` for the current auth status.",
            host=host,
            transport=transport,
        )
...
async def apply_tool_auth(server: Any, config: AuthConfig, modes: dict[str, str]) -> dict[str, str]:
    ...
    tools (``approval_respond``, ``easyhunt_load_scope``, …) are declared with a
    ...
    Returns ``{tool_name: scope}`` for the audit record and ``easyhunt doctor``.
    """
    if not (config.enabled and config.enforce_tool_scopes):
        return {}
...
def describe(config: AuthConfig) -> dict[str, Any]:
    """Auth posture, for ``easyhunt doctor`` and ``easyhunt_status``."""
    return {
        "mode": config.mode,
