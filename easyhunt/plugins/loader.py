...
from easyhunt.plugins.matchers import MatchInput, MatchResult, evaluate_rule
from easyhunt.plugins.schema import PluginManifest, validate_manifest

log = logging.getLogger("easyhunt.plugins")
...
# EasyHunt manifest of its own (raw Nuclei/Jaeles/Semgrep rules, BBOT presets).
_DIR_KINDS = {
    "nuclei": "nuclei",
    "jaeles": "jaeles",
    "semgrep": "semgrep",
...
class PluginRegistry:
    ...
    def _load_yaml(self, path: Path, root: Path) -> None:
        ...
        # A file with no EasyHunt manifest is a native rule for its directory's
    ...
    def load_python_tools(self) -> list[str]:
        ...
        expose capability through ``@easyhunt_tool`` — which means the control
        ...
        for manifest in self.by_kind("python-tool"):
            ...
            if not entry.is_file():
                ...
                continue
            module_name = f"easyhunt_plugin_{manifest.id.replace('-', '_').replace('.', '_')}"
            try:
