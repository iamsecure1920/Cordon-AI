...
BBOT already orchestrates 80+ recon modules behind a preset system, so Cordon
...
from cordon.control_plane.jobs import Job
from cordon.errors import ToolUnavailable
from cordon.knowledge.findings import Asset, Evidence, Finding, Severity, Status
from cordon.tools.base import ToolSpec, cordon_tool
from cordon.tools.common import register_spec

log = logging.getLogger("cordon.engines.bbot")
...
# Presets Cordon will run, and how each is classified. Anything not listed is
    ...
    *,
    ...
    try:
        ...
        raise ToolUnavailable(
            ...
            "'pip install cordon-ai[engines]') and re-run.",
            tool="bbot",
        ) from exc
...
async def _launch(
    ...
    *,
    ...
    if preset not in PRESET_MODES:
        raise ToolUnavailable(
            f"unknown BBOT preset {preset!r}; Cordon exposes {sorted(PRESET_MODES)}",
            tool="bbot",
        )
