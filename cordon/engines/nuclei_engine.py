"""Nuclei — the vulnerability core, and Cordon's primary rule DSL.
...
from cordon.tools.base import ToolSpec, cordon_tool, guarded_run
from cordon.tools.common import register_spec
from cordon.util.parse import parse_jsonl

log = logging.getLogger("cordon.engines.nuclei")
...
async def nuclei_scan(
    ...
    if tags:
        ...
        if overlap:
            raise ToolUnavailable(
                f"tags {sorted(overlap)} are disruptive and are never run by Cordon",
                tool="nuclei",
                tags=sorted(overlap),
            )
    ...
    # coverage of nothing.
    from cordon.tools.common import targets_or_assets
