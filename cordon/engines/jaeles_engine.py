...
log = logging.getLogger("cordon.engines.jaeles")
...
async def jaeles_scan(
    target: str, selector: str = "cordon/*", wait_seconds: float = 120.0
) -> dict[str, Any]:
    """Run Jaeles YAML signatures (plugin class C) against in-scope targets.

    selector picks signatures by name or glob, e.g. 'cordon/*' or 'cve/*'.
    ...
    if _signature_dir(engagement) is None:
        raise ToolUnavailable(
            ...
            "(or set rules.jaeles_dir); Cordon will not run signatures from an "
            "arbitrary path.",
            tool="jaeles",
        )
