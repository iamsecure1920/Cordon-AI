    ...
    for line in run.values:
        ...
        for match in matches:
            ...
            finding = Finding(
                ...
                how_found=f"Cordon rule-pack '{manifest.id}' matched via {match.matched_by}",
                source_tool="cordon-rules",
                rule_id=manifest.id,
                confidence=0.5,
...
async def waf_detect(target: str) -> dict[str, Any]:
    ...
    return {
        ...
        "tools": [run.to_dict()],
        "note": (
            "A WAF is context, not an obstacle to route around. Cordon will not "
