"""Catalog entries for tools Cordon knows about but does not drive directly.

These complete the tool table from the build guide. Each has an argument policy
and a recorded license, so ``cordon doctor`` reports them, the report's tool
...
``@cordon_tool`` — the policy and license are already here.
"""
...
from cordon.tools.common import HOST_PATTERN, PATH_PATTERN, URL_PATTERN, register_spec
