...
The gap this fills. Cordon drives 81 scanners well and finds almost nothing on
...
def _instructions(surface: dict[str, Any]) -> str:
    ...
    Cordon's L5 strategy layer is a model — the Claude CLI driving these tools
...
def _enrich(proposals: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    The technique index answers "how" — which Cordon tool tests the class a
...
async def hunt_plan(focus: str | None = None, limit: int = 120) -> dict[str, Any]:
    ...
    if not client.enabled:
        # Not a degraded path. Cordon's L5 strategy layer IS a model — the
    ...
    return {
        ...
        "note": (
            ...
            "`technique` field names the Cordon tool, vetted payload list and "
