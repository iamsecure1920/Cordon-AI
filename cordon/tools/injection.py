...
shipping the tool at all: ``cordon doctor`` reports a healthy install, the
...
``@cordon_tool`` scope-checks the ``target`` argument. A flag that reads a
...
but ``sandbox.default_image`` is ``cordon:latest``, so :meth:`Sandbox.plan`
...
from cordon.tools.common import URL_PATTERN, ToolRun, register_spec, run_one, split_targets
...
#: So no argument value anywhere in Cordon may contain ``&`` — which also means
...
#: The tagged user-agent (``Cordon-AI/2.0 (authorized-testing)``) carries
...
def _http_target(value: str, *, tool: str) -> str:
    ...
    if "&" in url:
        ...
        # relax it, so a two-parameter URL cannot reach any tool in Cordon.
        raise ValueError(
            f"{tool}: {url!r} carries more than one query parameter. '&' is refused in "
            "every Cordon argument value and cannot be relaxed per tool. Test one "
...
async def cmdi_probe(target: str, parameter: str | None = None, level: int = 1) -> dict[str, Any]:
    ...
    **Read the tool-availability note if this returns UNTESTED.** PyPI's
    ...
    argv = [
        ...
        # DISCARDS -u. Every Cordon run therefore scanned nothing: commix
        ...
        if not BODY_PAIR.fullmatch(data):
            return {
                ...
                "message": (
                    ...
                    "'&' is refused for every argument in Cordon, so a multi-parameter "
