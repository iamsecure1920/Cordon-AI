...
* **Scanning happens on files in the engagement workspace.** Cloning is a
  separate, audited step, so "what did Cordon read" always has an answer.
...
TRUFFLEHOG = register_spec(
    ToolSpec(
        ...
        # AGPL-3.0: relevant if Cordon is ever redistributed as a bundle.
        license="AGPL-3.0",
...
def _is_own_output(path: str) -> bool:
    """True when a hit points at Cordon's own logs, not a target artifact."""
    ...
    *candidate* until secret_validate says otherwise.
    ...
    gitleaks = await run_one(
        ...
        [
            ...
            # lives inside the Cordon-AI repo, so a workspace scan reported
...
async def source_fetch(repo_url: str, depth: int = 100) -> dict[str, Any]:
    ...
    did Cordon read". Only github/gitlab/bitbucket HTTPS URLs are accepted.
