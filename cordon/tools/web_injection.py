...
from cordon.tools.common import URL_PATTERN, split_targets, targets_or_assets
...
_CLASSES: dict[str, dict[str, Any]] = {
    ...
    "crlf": {
        ...
        "safe": "",
        "where": "headers",
        "signature": re.compile(r"cordon-injected", re.I),
        "follow": False,
        "severity": Severity.MEDIUM,
    ...
    "hpp": {
        ...
        "payloads": ["cordon-hpp-canary"],
        "duplicate": True,
        "safe": "",
        "where": "body",
        "signature": re.compile(r"cordon-hpp-canary"),
        "follow": False,
...
#: (``https%3A%2F%2Fcordon-canary.invalid…``) must not match: it is a URL
...
def _redirects_to_canary(location: str) -> bool:
    ...
    it. ``Location: http://www.victim.example/api?id=cordon-canary.invalid``
    ...
    destination hostname *is* the canary (``https://cordon-canary.invalid/…``,
    ``//cordon-canary.invalid/…``, and the ``///host`` form browsers
    ...
    if parsed.hostname and parsed.hostname.lower() == _CANARY:
        return True
    # ``///cordon-canary.invalid/…`` parses with no hostname but browsers
