...
from cordon.control_plane.approval import ApprovalGate, build_backend
...
from cordon.knowledge.findings import AssetStore, FindingStore
from cordon.knowledge.graphmemory import GraphMemory
from cordon.knowledge.memory import PoCMemory
from cordon.knowledge.neuron import NeuronBrain
...
class Engagement:
    ...
    def __init__(self, scope: Scope, config: Config, *, workspace: Path | None = None) -> None:
        ...
        from cordon.knowledge.sessions import SessionStore
        ...
        memory_path = config.path("memory.poc_store", "~/.cordon/poc-memory.jsonl")
        self.memory = PoCMemory(memory_path or Path.home() / ".cordon" / "poc-memory.jsonl")
        ...
        brain_path = config.path("memory.brain_store", "~/.cordon/neuron-brain.jsonl")
        activity_path = config.path("memory.brain_activity", "~/.cordon/brain-activity.jsonl")
        self.brain = NeuronBrain(
            brain_path or Path.home() / ".cordon" / "neuron-brain.jsonl",
            activity_path or Path.home() / ".cordon" / "brain-activity.jsonl",
        )
    ...
    def create(
        ...
        *,
        ...
        if resolved_scope is None:
            raise ConfigError(
                "no scope.yaml found — Cordon refuses to run without an authorization "
                "artifact. Copy scope.example.yaml, fill it in from the program policy, "
                "and pass --scope or set $CORDON_SCOPE."
            )
...
def get_engagement() -> Engagement:
    ...
    if _CURRENT is None:
        raise ConfigError(
            "no active engagement — load a scope first (cordon_load_scope, or "
