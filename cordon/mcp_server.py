"""Cordon MCP server — the control plane's only door.

Everything the agent can do arrives here. Capability modules register themselves
through :func:`cordon.tools.base.cordon_tool`, and this module walks the
...
from cordon.tools.base import REGISTRY, registered_tools

log = logging.getLogger("cordon.mcp")
...
CAPABILITY_MODULES = [
    "cordon.engines.bbot_engine",
    "cordon.engines.nuclei_engine",
    "cordon.engines.jaeles_engine",
    "cordon.engines.semgrep_engine",
    ...
    "cordon.tools.js_analysis",
    "cordon.tools.pattern_scan",
    "cordon.tools.ports",
    "cordon.tools.takeover",
    "cordon.tools.secrets",
    "cordon.tools.contracts",
    "cordon.tools.cloud",
    "cordon.tools.exploitation",
    "cordon.tools.injection",
    "cordon.tools.web_injection",
    "cordon.tools.exploit_chain",
    "cordon.tools.webscan",
    "cordon.tools.waf_bypass",
    "cordon.tools.prompts",
    "cordon.tools.burp",
    "cordon.tools.code_audit",
    "cordon.tools.hunt_plan",
    "cordon.tools.auth_crawl",
    "cordon.tools.auth_surface",
    "cordon.tools.sessions",
    "cordon.tools.smuggling_framework",
    "cordon.tools.forbidden",
    "cordon.tools.research_guide",
    "cordon.tools.browser_verify",
    "cordon.tools.llmsec",
...
mcp: FastMCP = FastMCP(
    name="cordon",
    instructions=(
        "Cordon AI — authorized VAPT orchestration.\n\n"
...
async def cordon_status() -> dict[str, Any]:
    engagement = current_engagement()
    if engagement is None:
        return {
            "ok": False,
            "loaded": False,
            "message": "No scope loaded. Call cordon_load_scope first.",
        }
...
async def cordon_finish(outcome: str = "completed") -> dict[str, Any]:
    engagement = current_engagement()
    if engagement is None:
...
async def job_fetch(job_id: str, wait_seconds: float = 0) -> dict[str, Any]:
    try:
        ...
        return _err(exc)
    from cordon.util.parse import cap_payload
...
async def graph_recall(subject: str, depth: int = 1, limit: int = 60) -> dict[str, Any]:
    ...
    from cordon.knowledge.graphmemory import ingest_engagement
...
@mcp.tool(
    ...
    description=(
        ...
        "workspace files directly. The CLI mirrors this as `cordon dashboard`."
    ),
    tags={"control", "reporting"},
    annotations={"readOnlyHint": True, "openWorldHint": False},
)
async def dashboard_state() -> dict[str, Any]:
    from cordon.tools.dashboard import collect_state
...
async def rules_list(kind: str | None = None, phase: str | None = None) -> dict[str, Any]:
    from cordon.plugins.loader import get_registry
...
@mcp.tool(
    name="cordon_capabilities",
    description="List registered capabilities with their phase, mode, and origin.",
    tags={"control"},
    annotations={"readOnlyHint": True, "openWorldHint": False},
)
async def cordon_capabilities(phase: str | None = None, mode: str | None = None) -> dict[str, Any]:
...
def build_server(
    *,
    ...
    if scope_path:
        ...
        try:
            from cordon.control_plane.pins import verify_or_write_pins
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="cordon-mcp", description="Cordon AI MCP server (authorized testing only)"
    )
