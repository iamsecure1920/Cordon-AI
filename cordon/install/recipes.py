...
class Recipe:
    ...
    python_max: str = ""
    license: str = "unknown"
    #: Surfaced by `cordon install` and recorded in the report.
    caveat: str = ""
    #: Core tools make up the default pipeline; the rest are opt-in.
    core: bool = False
    #: Cordon *imports* this one rather than shelling out to it, so it has to
    #: live in Cordon's own environment. Only bbot qualifies: the recon engine
...
# --------------------------------------------------------------------------- #
# Engines — the tools Cordon is built around
# --------------------------------------------------------------------------- #
...
_add(Recipe(tool="bbot", method="pip", package="bbot>=2.4", category="engine", core=True,
            ...
            caveat=(
                "AGPL-3.0. Installed into Cordon's environment rather than isolated, "
...
_add(Recipe(tool="strix", method="manual", package="", category="engine", license="Apache-2.0",
            caveat=(
                "Autonomous exploitation agent — Cordon runs it only inside the "
...
_add(_pd("httpx", "httpx/cmd/httpx", category="http", core=True,
         caveat=(
             "Name collision with the Python httpx CLI. Cordon resolves this by "
...
_add(Recipe(tool="gf", method="go", package="github.com/tomnomnom/gf@latest",
            ...
            caveat=(
                ...
                "Cordon's pattern_scan runs the same vetted rules/gf/ patterns "
...
_add(Recipe(tool="nmap", method="apt", package="nmap", category="ports", license="NPSL",
            ...
            caveat="-sS SYN scan needs root; Cordon defaults to safe NSE categories only."))
_add(Recipe(tool="masscan", method="apt", package="masscan", category="ports",
            ...
            caveat="AGPL-3.0. Always needs raw sockets. Rate is hard-capped by Cordon."))
...
_add(Recipe(tool="slither", method="pipx", package="slither-analyzer",
            ...
            caveat="AGPL-3.0 — matters if an Cordon bundle is redistributed. "
...
_add(Recipe(tool="sqlmap", method="pipx", package="sqlmap", category="exploit",
            license="GPL-2.0", core=True,
            caveat="Cordon hard-blocks --dump, --dbs, --tables, --os-shell, --file-read."))
...
# --------------------------------------------------------------------------- #
#
# These ship in the container image. They are catalogued here so `cordon doctor`
...
_add(Recipe(tool="smuggler-framework", method="manual", package="",
            ...
            caveat=(
                ...
                "CORDON_SMUGGLER_FRAMEWORK at its directory, and it is bind-mounted "
...
_add(Recipe(tool="nosqli", method="go", category="exploit", license="AGPL-3.0",
            ...
            caveat=(
                "AGPL-3.0 — matters the moment an Cordon bundle is redistributed. "
