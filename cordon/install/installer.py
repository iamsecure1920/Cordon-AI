...
same identity check ``cordon doctor`` uses, so "installed" means "verified
...
reached through ``cordon install`` or ``cordon doctor --fix``, both of which
...
from cordon.control_plane.sandbox import PROBE_TIMEOUT
from cordon.install.recipes import RECIPES, SYSTEM_PACKAGES, Recipe, install_order

log = logging.getLogger("cordon.install")
...
#
# — and `cordon doctor` reported all four with a green tick. That is the same
...
def _probe_library(spec: Any) -> ToolHealth:
    """A tool Cordon imports rather than executes (bbot) — import it for real.
...
def health_check(
    ...
    *,
    ...
    """
    from cordon.tools.common import CATALOG, resolve_binary, verify_identity
...
class Installer:
    ...
    def _run(self, command: str, *, env: dict[str, str] | None = None, timeout: int | None = None):
        # Hard guard: nothing may install into the interpreter running Cordon.
        ...
        if (
            ...
            raise RuntimeError(
                "refusing to install into Cordon's own environment — "
    ...
    def _verify(tool: str, *, execute: bool = True) -> tuple[bool, str]:
        """Post-install check using the same probe as ``cordon doctor``.
    ...
    def _command_for(self, recipe: Recipe) -> str:
        ...
        if method in {"pip", "pipx"}:
            if recipe.library:
                # The one exception: Cordon imports this, so it must share our
            ...
            # Cordon's venv pulled in fastmcp-slim and silently removed
    ...
    def _release_script(self, recipe: Recipe) -> str:
        ...
        tmp = f"/tmp/cordon-install-{recipe.tool}"  # noqa: S108 — scratch, removed below
