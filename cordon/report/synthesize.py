...
from cordon.report.templates import (
    AUDIT_NOTE,
    CONFIRMED_INTRO,
    EMPTY_CONFIRMED,
    NEEDS_REVIEW_INTRO,
...
def _markdown(engagement: Any, summary: dict[str, Any], executive: str) -> str:
    ...
    parts: list[str] = [
        ...
        "",
        f"*Generated {datetime.now(UTC).strftime('%Y-%m-%d %H:%M UTC')} by Cordon AI*",
        "",
        "## Authorization",
        "",
...
def _tool_inventory(engagement: Any) -> list[tuple[str, Any, str, bool]]:
    ...
    from cordon.tools.common import CATALOG, installed
...
async def generate_report(
    ...
    *,
    ...
    try:
        ...
        from cordon.llm.summarize import summarize_findings
    ...
    try:
        from cordon.report.graphs import render_attack_paths, render_task_graph
