...
log = logging.getLogger("cordon.graphmemory")
...
class GraphMemory:
    ...
    def observe_asset(self, asset: Any) -> Entity:
        """Ingest an :class:`~cordon.knowledge.findings.Asset`."""
        entity = self.observe(
            Entity(
    ...
    def _mirror_relationship(self, edge: Relationship) -> None:
        ...
        try:
            with self._driver.session() as session:
                ...
                session.run(
                    "MATCH (a:CordonEntity {id: $source}), (b:CordonEntity {id: $target}) "
